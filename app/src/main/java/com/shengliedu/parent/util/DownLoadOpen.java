...
public class DownLoadOpen {
	...
	public void downOpen(final String url, final String name,final int size) {
		String namereString = name;
		if (namereString.contains("/")) {
			namereString = name.substring(name.lastIndexOf("/") + 1);
		}
		File file = new File(pathName + namereString);
		Log.v("TAG", "yes1="+pathName + namereString);
		if (file.exists()) {
	...
	private void PushDeliog(final String url, final String name, final int size) {
	 ...
	 * 02 * 从服务器中下载
	 */
	protected void downLoadFile(final String url, final String name,final int size) {
	...
	private File getFileFromServer(String path, String name, ProgressDialog pd,int size)
			throws Exception {
