...
import com.shengliedu.parent.chat.util.FormatTools;
import com.shengliedu.parent.chat.util.PinyinComparator;
import com.shengliedu.parent.chat.util.Tool;
import com.shengliedu.parent.util.SharedPreferenceTool;
import com.shengliedu.parent.util.UnicodeConverter;
...
public class XmppConnection {
	...
	public void sendMsg(String chatName, String msg, int chatType)
		...
		if (msg.isEmpty()) {
			Tool.initToast(App.getInstance(), "随便写点什么呗");
		} else {
			if (chatType == ChatItem.CHAT) {
				ChatManager cm = XmppConnection.getInstance().getConnection()
						.getChatManager();
	...
	public VCard getUserInfo(String user) { // null 时查自己
		try {
			...
			if (user == null) {
				...
				Log.v("TAG", "VCard:" + user + "@" + Constants.SERVER_NAME);
	...
	public MultiUserChat createRoom(String roomName) {// String user,
		...
		try {
			...
			Tool.initToast(App.getInstance(), "网络不给力,请重试");
	...
	public MultiUserChat joinMultiUserChat(String user, String roomsName,
		...
		try {
			...
			Date date = null;
			if (!TextUtils.isEmpty(SharedPreferenceTool.getChatTime(App
					.getInstance()))) {
				date = new Date(Long.parseLong(SharedPreferenceTool
						.getChatTime(App.getInstance())));
			} else {
				date = new Date();
			}
			...
			// 用户加入聊天室
			Message oldMsg = muc.nextMessage(5000);
