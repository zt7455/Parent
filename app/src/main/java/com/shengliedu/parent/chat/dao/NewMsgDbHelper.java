package com.shengliedu.parent.chat.dao;
...
public class NewMsgDbHelper {             //新消息条数
	...
	private class SqlLiteHelper extends SQLiteOpenHelper {
		...
		private static final String DB_NAME = "pnewMsg";
		...
		public void onCreate(SQLiteDatabase db) {   //MsgId对聊天来说即对方id
			String sql = "CREATE TABLE  IF NOT EXISTS pnewMsg"
		...
		private void dropTable(SQLiteDatabase db) {
			String sql = "DROP TABLE IF EXISTS pnewMsg";
			db.execSQL(sql);
		}
	...
	public int getMsgCount(String msgId){   
		int count = 0 ;
		String sql ="select msgCount from pnewMsg where msgId=? and whosMsg=?";
	...
	public int getMsgCount(){   
		int count = 0 ;
		String sql ="select sum(msgCount) from pnewMsg where whosMsg=? and msgId != 0";
