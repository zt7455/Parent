...
import com.shengliedu.parent.adapter.BookPageAdapter;
import com.shengliedu.parent.app.App;
import com.shengliedu.parent.bean.TeachBookContent;
...
public class TeachBookPageActivity extends BaseActivity {
	...
	protected void onPause() {
		...
		if (position!=0) {
			SharedPreferenceTool.setUserBookPage(this,App.userInfo.id+"_"+bookId, App.userInfo.id+"_"+bookId+"_"+position);
		}
	}
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what==1){
				...
				if (mainbookContent!=null && mainbookContent.size()>0) {
					...
					if (!App.listIsEmpty(bookContent) && !App.listIsEmpty(urls)) {
						...
						if (App.listIsEmpty(bookContent)) {
							toast("目录为空");
						}
						if (App.listIsEmpty(urls)) {
							toast("书页为空");
						}
					}
				}
...
//						urls.add(dataBook.get(i).host+dataBook.get(i).link);
				...
				String user_book_page=SharedPreferenceTool.getUserBookPage(TeachBookPageActivity.this,App.userInfo.id+"_"+bookId);
