...
public class XYImageView extends ImageView {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (width > BaseActivity.screenwidth) {
			// 如果图宽大于屏宽，就不用水平居中
			center(false, true);
		} else {
			center(true, true);
	...
	public XYImageView(Context context, int imageWidth, int imageHeight) {
		super(context);
	...
	public XYImageView(Context context, AttributeSet attrs, int imageWidth,
			int imageHeight) {
		super(context, attrs);
		this.imageHeight = imageHeight;
		this.imageWidth = imageWidth;
	...
	private void arithScaleRate() {
		float scaleWidth = BaseActivity.screenwidth / (float) imageWidth;
		float scaleHeight = BaseActivity.screenHigh / (float) imageHeight;
	...
	public void setImageBitmap(Bitmap bitmap) {
		...
		arithScaleRate();
		// 缩放到屏幕大小
		zoomTo(scaleRate, BaseActivity.screenwidth / 2f,
				BaseActivity.screenHigh / 2f);
		// 居中
		layoutToCenter();
	}
	...
	public void layoutToCenter() {
		...
		float height = imageHeight * getScale();
		// 空白区域宽高
		float fill_width = BaseActivity.screenwidth - width;
		float fill_height = BaseActivity.screenHigh - height;
		// 需要移动的距离
		float tran_width = 0f;
	...
	protected float getValue(Matrix matrix, int whichValue) {
		...
		mMinZoom = (BaseActivity.screenwidth / 2f) / imageWidth;
