...
public class HomeWorkDateAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		viewHolder.after_time.setText(list.get(position).date);
		try {
		...
		String left_count = list.get(position).left;
		viewHolder.after_time.setText(list.get(position).date);
		...
		String has_view = list.get(position).hasview;

		if ("1".equals(has_view)) {
			viewHolder.yiyue.setText("[已阅]");
			viewHolder.yiyue.setTextColor(Color.GRAY);
		} else {
			viewHolder.yiyue.setText("[未阅]");
			viewHolder.yiyue.setTextColor(Color.RED);
		}
